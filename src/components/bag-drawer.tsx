...
import { ArrowRight, Check, CheckCircle2, ChevronLeft, Download, Minus, Plus, ShoppingBag, Trash2, Upload, WalletCards } from "lucide-react";
...
export function BagDrawer({ open, onClose, onAuth }: { open: boolean; onClose: () => void; onAuth: (intent: string) => void }) {
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === "Escape") finishOrder();
    };
    window.addEventListener("keydown", closeOnEscape);
    return () => window.removeEventListener("keydown", closeOnEscape);
  }, [finishOrder, successOpen]);
  ...
  const placeOrder = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    else { setReceiptDetails(details); setOrder(data.order); }
    setSubmitting(false);
  };
  ...
  const confirmOrder = async () => {
    if (!order || !proofName || submitting || finalizing) return;
    setFinalizing(true);
    setError("");
    try {
      await clearBag();
      ...
      setError("Your order was placed, but the bag could not be cleared. Please refresh and try again.");
    } finally {
      setFinalizing(false);
    }
  };
  ...
  </Modal>{successOpen && <div className="order-success-layer" role="presentation"><section className="order-success-dialog" role="dialog" aria-modal="true" aria-labelledby="order-success-title"><span className="order-success-icon"><CheckCircle2 /></span><p className="eyebrow dark">Payment proof received</p><h2 id="order-success-title">Your order has been placed</h2><p>Your payment is awaiting verification. We will notify you as your order progresses.</p><button ref={successButtonRef} type="button" className="primary-button" onClick={finishOrder}>Return to the maison <ArrowRight /></button></section></div>}</>;
}
