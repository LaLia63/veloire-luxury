...
function Experience({ products, departments }: { products: Product[]; departments: Department[] }) {
  ...
  const activeDepartment = departments.find((item) => item.slug === department) ?? departments[0];
  const activeDepartmentSlug = activeDepartment?.slug ?? "";
  const activeCategory = category && activeDepartment?.categories.some((item) => item.slug === category) ? category : null;
  const selected = useMemo(() => products.find((product) => product.id === selectedId) ?? null, [products, selectedId]);
  const visibleProducts = useMemo(() => products.filter((product) => product.department?.slug === activeDepartmentSlug && (!activeCategory || product.category?.slug === activeCategory)), [products, activeDepartmentSlug, activeCategory]);
  ...
  const wishlist = async (product: Product) => {
    ...
    try {
      ...
      setWishlistNotice(result === "added" ? `${product.name} saved to wishlist` : `${product.name} removed from wishlist`);
    } catch {
      setWishlistNotice("Wishlist could not be updated. Please try again.");
    }
  };

  useEffect(() => {
    if (!wishlistNotice) return;
    const timer = window.setTimeout(() => setWishlistNotice(""), 2200);
  ...
  }, [department, category, visibleProducts]);

  const refreshTimer = useRef<number | null>(null);
  const lastFallbackRefresh = useRef(0);
  const scheduleCatalogRefresh = useCallback((delay = 180) => {
  ...
  useEffect(() => {
    ...
    const refresh = () => scheduleCatalogRefresh();
    const channel = supabase
      .channel("vlr-public-catalog")
      .on("postgres_changes", { event: "*", schema: "public", table: "vlr_products" }, refresh)
      .on("postgres_changes", { event: "*", schema: "public", table: "vlr_product_variants" }, refresh)
      .on("postgres_changes", { event: "*", schema: "public", table: "vlr_categories" }, refresh)
      .on("postgres_changes", { event: "*", schema: "public", table: "vlr_departments" }, refresh)
      .subscribe();

    // Availability changes and deletes can be hidden by RLS. Refresh at most
    // once per 30 seconds when the visitor returns, covering missed events
    // without polling while the tab is idle.
  ...
  return <main>
    ...
    <section className="hero" id="top"><Image src="/images/campaign/veloire-hero.webp" alt="Woman in an oxblood evening gown inside a cinematic stone gallery" fill preload sizes="100vw"/><div className="hero-shade"/><div className="hero-copy"><p className="eyebrow">La Collection No. 01 · 2026</p><h1>Atelier<br/><em>Nocturne</em></h1><p>Form, shadow and quiet confidence.</p><a className="text-link" href="#shop">Discover the collection <ArrowUpRight aria-hidden="true" /></a></div><span className="hero-index">V / 01</span></section>

    <section className="shop-preview" id="shop" data-reveal><div className="section-heading"><div><p className="eyebrow dark">The edit</p><h2>Objects of<br/><em>presence</em></h2></div><p>Considered silhouettes and enduring objects, composed for a life lived with intention.</p></div><div className="department-tabs">{departments.map((item) => <button className={item.slug === activeDepartmentSlug ? "active" : ""} key={item.id} onClick={() => { setDepartment(item.slug); setCategory(null); }}>{item.name}</button>)}</div><div className="category-tags"><button className={!activeCategory ? "active" : ""} onClick={() => setCategory(null)}>View all</button>{activeDepartment?.categories.map((item) => <button className={item.slug === activeCategory ? "active" : ""} key={item.id} onClick={() => setCategory(item.slug)}>{item.name}</button>)}</div><div className="product-grid">{visibleProducts.length ? visibleProducts.map((product, index) => { const saved = wishlistIds.includes(product.id); return <article className={`product-card card-${(index % 3) + 1}`} key={product.id} data-reveal><button className="product-image" onClick={() => setSelectedId(product.id)}><Image src={product.primary_image_url} alt={product.name} fill sizes="(max-width: 760px) 100vw, 33vw"/><span>{product.is_new_arrival ? "New arrival" : product.category?.name}</span></button><div className="product-meta"><button className="product-title" onClick={() => setSelectedId(product.id)}><h3>{product.name}</h3><p>{formatMMK(product.base_price)}</p></button><button className={saved ? "wishlist-active" : ""} aria-label={saved ? `Remove ${product.name} from wishlist` : `Save ${product.name} to wishlist`} aria-pressed={saved} onClick={() => void wishlist(product)}><Heart fill={saved ? "currentColor" : "none"} /></button></div></article>; }) : <div className="no-results"><p>No pieces are currently presented in this edit.</p><button onClick={() => setCategory(null)}>View the full department</button></div>}</div></section>

    <section className="campaign-split" id="collections" data-reveal><div className="campaign-image"><Image src="/images/campaign/veloire-men.webp" alt="Man in precise noir tailoring inside a stone gallery" fill sizes="(max-width: 800px) 100vw, 50vw"/></div><div className="campaign-copy"><p className="eyebrow">Collection II</p><h2>The quiet<br/><em>form</em></h2><p>Tailoring distilled to line, proportion and material. Designed to speak softly—and remain.</p><button onClick={() => { setDepartment("men"); setCategory(null); document.getElementById("shop")?.scrollIntoView(); }}>Enter menswear <ArrowRight /></button></div></section>
    <section className="services" data-reveal><article><span>01</span><h3>Private curation</h3><p>Your AI stylist understands the live collection and may compose, compare, add, remove or refine your bag in English and မြန်မာ.</p><button onClick={() => setAiOpen(true)}>Consult the stylist</button></article><article><span>02</span><h3>VÉLOIRE Privilege</h3><p>Receive 10 points for every 200,000 MMK of a verified order, with considered redemption on future pieces.</p><button onClick={() => user ? setAccountOpen(true) : askAuth("join VÉLOIRE Privilege")}>View your privilege</button></article><article><span>03</span><h3>From atelier to arrival</h3><p>Follow preparation, quality inspection, signature packaging and delivery from your private account.</p><button onClick={() => user ? setAccountOpen(true) : askAuth("track your order")}>Track an order</button></article></section>
    <section className="objects-banner" data-reveal><Image src="/images/campaign/veloire-accessories.webp" alt="Oxblood handbag, minimalist watch and fragrance" fill sizes="100vw"/><div><p className="eyebrow">Objets Privés</p><h2>Rituals,<br/><em>refined.</em></h2><button onClick={() => { setDepartment("women"); setCategory("handbags"); document.getElementById("shop")?.scrollIntoView(); }}>Discover accessories</button></div></section>
    ...
    <ProductModal key={selected?.id ?? "closed"} product={selected} onClose={() => setSelectedId(null)} onAuth={askAuth} onBag={() => setBagOpen(true)}/>
